...
class KEBClient {
  ...
  async createBinding(instanceID, bindingID, expirationSeconds = DEFAULT_EXPIRATION_SECONDS) {
    const payload = {
      service_id: KYMA_SERVICE_ID,
      plan_id: this.planID,
      parameters: {
  ...
  async deleteBinding(instanceID, bindingID) {
    const endpoint = `service_instances/${instanceID}/service_bindings/${bindingID}?accepts_incomplete=true&service_id=${KYMA_SERVICE_ID}&plan_id=${this.planID}`;
